...
WORKDIR /src
COPY ["DonateCraft.csproj", "DonateCraft/"]
RUN dotnet restore "DonateCraft/DonateCraft.csproj"

WORKDIR "/src/DonateCraft"
COPY . .
RUN dotnet build "DonateCraft.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "DonateCraft.csproj" \
...
Execute unit tests
```
    cd "DonateCraft/test/DonateCraft.Tests"
    dotnet test
```

Deploy application
```
    cd "DonateCraft/src/DonateCraft"
